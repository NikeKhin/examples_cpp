...
int main()
{
    ...
    for(int i=0;i<size;i++)
        for(int j=0;j<size;j++)
        {
            ...
            frame.point(i,j).green( (i+j)/4 );  // добавл€ем немного зеленого
    ...
    std::normal_distribution<double> distribution(0.0, 0.5); // нормальное распределение со средним 127 и — ќ 32
    for(int i=0;i<size;i++)
        for(int j=0;j<size;j++)
        {
            frame.point(i,j).red  ( distribution(generator) );    // заливка красным
            frame.point(i,j).green( distribution(generator) );  // добавл€ем немного зеленого
